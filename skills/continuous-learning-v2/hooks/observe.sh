...
# AntiGravity Agent passes hook data via stdin as JSON.
#
...
# Can also be registered manually in ~/.agents/settings.json.

set -e

# Hook phase from CLI argument: "pre" (PreToolUse) or "post" (PostToolUse).
# Manual settings.json installs can call this script without the plugin
# wrapper's positional phase argument, but AntiGravity Agent still exposes the hook
# event name in AntiGravity_HOOK_EVENT_NAME.  Fall back to that env var before
...
if [ -z "$HOOK_PHASE" ]; then
  case "${AntiGravity_HOOK_EVENT_NAME:-}" in
...
# Read JSON from stdin (AntiGravity Agent hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  if [ -n "$_GIT_ROOT" ]; then
    export AntiGravity_PROJECT_DIR="$_GIT_ROOT"
    unset CLV2_NO_PROJECT
  else
    unset AntiGravity_PROJECT_DIR
...
#   - Monster AntiGravity observing its own Haiku observer sessions (self-loop)
#   - Monster AntiGravity observing other tools' automated sessions
...
# Pass HOOK_PHASE via env var since AntiGravity Agent does not include hook type in stdin JSON
...
try:
    ...
    # AntiGravity Agent does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - AntiGravity Agent hook format
