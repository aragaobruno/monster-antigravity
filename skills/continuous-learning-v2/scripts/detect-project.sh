...
_CLV2_HOMUNCULUS_DIR="$(_ag_resolve_homunculus_dir)"
...
_clv2_detect_project() {
  ...
  if [ -n "$AntiGravity_PROJECT_DIR" ] && [ -d "$AntiGravity_PROJECT_DIR" ] && command -v git &>/dev/null; then
    project_root=$(git -C "$AntiGravity_PROJECT_DIR" rev-parse --show-toplevel 2>/dev/null || true)
