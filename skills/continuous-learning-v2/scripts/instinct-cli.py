...
def _resolve_homunculus_dir() -> Path:
    ...
    if override:
        ...
        print(f"[monster-antigravity] CLV2_HOMUNCULUS_DIR={override!r} is not absolute; ignoring", file=sys.stderr)

    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg:
        if Path(xdg).is_absolute():
            return Path(xdg) / "monster-antigravity-homunculus"
        print(f"[monster-antigravity] XDG_DATA_HOME={xdg!r} is not absolute; ignoring", file=sys.stderr)

    return Path.home() / ".local" / "share" / "monster-antigravity-homunculus"
...
def _fetch_import_url(source: str, *, max_bytes: int = 2 * 1024 * 1024) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Monster AntiGravity-instinct-import/2"})
...
def _warn_legacy_data() -> None:
    """Warn if legacy ~/.agents/homunculus/ contains data while the active
    path has moved to the XDG directory."""
    legacy_dir = Path.home() / ".agents" / "homunculus"
...
def cmd_projects(args) -> int:
    ...
    if not registry:
        ...
        print("Projects are auto-detected when you use AntiGravity Agent in a git repo.")
        return 0
