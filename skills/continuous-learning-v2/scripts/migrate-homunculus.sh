#!/usr/bin/env bash
# One-shot migration from the legacy AntiGravity config tree into the
...
NEW="$(_ag_resolve_homunculus_dir)"
...
if command -v pgrep >/dev/null 2>&1; then
  if pgrep -f "${HOME}.*observer-loop\\.sh" >/dev/null 2>&1; then
    ...
    echo "Exit all AntiGravity Agent sessions, then re-run." >&2
    exit 1
  fi
else
...
Advisory: ~/.agents/settings.json still sets CLV2_CONFIG under the old path.
