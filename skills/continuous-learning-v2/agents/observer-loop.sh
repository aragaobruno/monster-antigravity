...
# parallel AntiGravity analysis processes.

set +e
unset AntiGravityCODE
...
analyze_observations() {
  ...
  if ! command -v AntiGravity >/dev/null 2>&1; then
    echo "[$(date)] AntiGravity CLI not found, skipping analysis" >> "$LOG_FILE"
    return
  fi
  ...
  # prefixes (e.g. /c/Users/...) that the AntiGravity subprocess cannot resolve.
  analysis_relpath=".observer-tmp/$(basename "$analysis_file")"

  prompt_file="$(mktemp "${observer_tmp_dir}/monster-antigravity-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  # Read the prompt into memory before the AntiGravity subprocess is spawned.
  # On Windows/MSYS2, the mktemp path can differ from the shell's later path
  # resolution, so relying on cat "$prompt_file" inside the AntiGravity invocation
  ...
  (
    ...
    if kill -0 "$AntiGravity_pid" 2>/dev/null; then
      echo "[$(date)] AntiGravity analysis timed out after ${timeout_seconds}s; terminating process" >> "$LOG_FILE"
      kill "$AntiGravity_pid" 2>/dev/null || true
    fi
  ) &
  watchdog_pid=$!

  wait_for_AntiGravity_analysis "$AntiGravity_pid"
  exit_code=$?
